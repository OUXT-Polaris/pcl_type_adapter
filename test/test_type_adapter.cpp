...
public:
  using AdaptedType =
    rclcpp::TypeAdapter<boost::shared_ptr<PCL_POINTCLOUD_TYPE>, sensor_msgs::msg::PointCloud2>;
  explicit PubNode(const rclcpp::NodeOptions & options) : Node("test", options)
  {
    publisher_ = create_publisher<AdaptedType>("pointcloud", 1);
  }
  void publish(const boost::shared_ptr<PCL_POINTCLOUD_TYPE> & point_cloud)
  {
...
public:
  ...
  explicit SubNode(
    ...
    const std::function<void(const boost::shared_ptr<PCL_POINTCLOUD_TYPE> &)> function)
  : Node("test", options)
  {
...
#define DEFINE_PUB_SUB_TEST(POINT_TYPE)                                                          \
  ...
  {                                                                                              \
    ...
    const auto pub_point_cloud =                                                                 \
      boost::shared_ptr<pcl::PointCloud<pcl::POINT_TYPE>>(new pcl::PointCloud<pcl::POINT_TYPE>); \
    auto sub_node = std::make_shared<SubNode<pcl::PointCloud<pcl::POINT_TYPE>>>(                 \
      options, [&](const boost::shared_ptr<pcl::PointCloud<pcl::POINT_TYPE>> & point_cloud) {    \
